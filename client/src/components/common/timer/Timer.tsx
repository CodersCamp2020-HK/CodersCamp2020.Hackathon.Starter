  ...
  linearProgress: {
    ...
    '& .MuiLinearProgress-barColorSecondary': {
      ...
      backgroundColor: ({ time }: { time: number; countProgress: number }) =>
        time <= 0 ? theme.palette.error.main : theme.palette.secondary.main,
    },
  },
  textWrapper: {
    width: '100%',
...
export const convertSeconds = (sec: number) => {
  ...
  return {
    ...
    seconds: seconds < 10 ? `0${Math.ceil(seconds)}` : `${Math.ceil(seconds)}`,
  };
};
    ...
    const timer = setInterval(() => {
      ...
      setProgress((oldProgress) => {
        if (countProgress === 1 && time >= 1) {
          return oldProgress === 100 ? 0 : oldProgress + 100 / (15 * 60);
        }
        if (time >= 1) {
          return oldProgress === 100 ? 0 : oldProgress + 100 / timeInSeconds;
        }
        return 100;
      });
    }, 1000);
    ...
    <div className={classes.contentWrapper}>
      <div className={classes.timerWrapper}>
        <div className={classes.textWrapper}>
          ...
          <p>{`Czas spotkania: ${convertSeconds(Math.abs(totalTime)).minutes}:${
            convertSeconds(Math.abs(totalTime)).seconds
          }`}</p>
        </div>
        <LinearProgress
